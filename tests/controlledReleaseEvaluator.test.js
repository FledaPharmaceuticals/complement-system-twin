...
test("marks a fully compliant candidate ready without activating it", async () => {
  const decision = await evaluateControlledRelease(await fixture());
...
test("blocks a policy provenance hash that does not match the policy manifest", async () => {
  const input = await fixture();
  input.calibrationRun.provenance.policyHash = "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
